import argparse, ast, io, json, os, re, sys, time, textwrap, multiprocessing as mp
...
SUPPORTED_CODE_EVAL_DATASETS = {"LiveCodeBench", "LiveBench"}


def validate_code_eval_dataset(dataset, data_type):
    if data_type != "code" or dataset not in SUPPORTED_CODE_EVAL_DATASETS:
        supported = ", ".join(sorted(SUPPORTED_CODE_EVAL_DATASETS))
        raise ValueError(f"Unsupported code eval dataset/type: {dataset} {data_type}. Supported code datasets: {supported}")


def validate_code_eval_records(data, dataset):
    for idx, item in enumerate(data):
        method = item.get("test_method")
        if method == "stdio":
            missing = [k for k in ("test_input", "test_output", "test_time_limit") if k not in item]
        elif method == "function":
            missing = [k for k in ("test_list",) if k not in item]
        else:
            missing = ["test_method"]
        if missing:
            raise ValueError(
                f"{dataset} item {idx} is missing required code-eval fields: {missing}. "
                "Use the full benchmark JSON/output with test cases, not a rolling slim prompt file."
            )


def extract_code_from_output(full_output: str) -> str | None:
...
def repair_extracted_code(data: list[dict]) -> int:
    repaired = 0
    failure_markers = (
        "Can not extract the answer!",
        "We can not extract the code in the output. ",
    )
    for item in data:
        full_outputs = item.get("full_output", [])
        extracted_outputs = item.get("extracted_output", [])
        for i, extracted in enumerate(extracted_outputs):
            if extracted not in failure_markers or i >= len(full_outputs):
                continue
            code = extract_code_from_output(full_outputs[i])
            if code:
                extracted_outputs[i] = code
                repaired += 1
    return repaired
...
def main():
    cfg          = get_config()
    validate_code_eval_dataset(cfg.dataset.eval_dataset, cfg.dataset.data_type)
    ...
    outputs_name = "eval-" + cfg.outputname + "-" + cfg.dataset.eval_dataset
    if num_node > 1:
    ...
    with open(file_name, 'r') as f:
        data = json.load(f)
    validate_code_eval_records(data, cfg.dataset.eval_dataset)
    repaired = repair_extracted_code(data)
    if repaired:
        print(f"[execute] repaired {repaired} code extraction failures from full_output", flush=True)
