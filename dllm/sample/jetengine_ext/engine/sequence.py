...
class Sequence:
    ...
    num_tokens_backend = 0
    block_length=4

    def __init__(self, prompt_token_ids: list[int], mask_token_id: int, sampling_params=SamplingParams(), sparsity_ratio: float = 0):
        ...
        self.update_kv_cache = 0
        self.no_update_kv_cache = 0
    ...
    def __getstate__(self):
        # Simplified for multiprocessing; customize as needed
        sp=None
        if self.sparsity_params is not None:
            if "need_update_kvcache_idx" in self.sparsity_params:
                sp={"dirty_tokens":self.sparsity_params["dirty_tokens"],"is_saving":self.sparsity_params.get("is_saving", False),
                "need_update_kvcache_idx":self.sparsity_params["need_update_kvcache_idx"],
                "no_need_update_kvcache_idx":self.sparsity_params["no_need_update_kvcache_idx"],"current_denoising_step":self.sparsity_params["current_denoising_step"],
                "dirty_token_mask":self.sparsity_params.get("dirty_token_mask", 0),
                "dirty_token_count":self.sparsity_params.get("dirty_token_count", len(self.sparsity_params["dirty_tokens"]))}
            else:
                sp={"dirty_tokens":self.sparsity_params["dirty_tokens"],"current_denoising_step":self.sparsity_params["current_denoising_step"],
                "dirty_token_mask":self.sparsity_params.get("dirty_token_mask", 0),
                "dirty_token_count":self.sparsity_params.get("dirty_token_count", len(self.sparsity_params["dirty_tokens"]))}
        return (self.seq_id, self.status, self.token_ids, self.num_tokens, self.num_prompt_tokens, 
                self.num_cached_tokens, self.block_table, self.intermediate_block_tokens, self.current_denoising_step,
                self.first_unmask_steps, self.block_first_unmask_steps, self.global_denoising_step, self.num_tokens_backend, sp, self.block_length)

    def __setstate__(self, state):
        (self.seq_id, self.status, self.token_ids, self.num_tokens, self.num_prompt_tokens, 
         self.num_cached_tokens, self.block_table, self.intermediate_block_tokens, self.current_denoising_step,
         self.first_unmask_steps, self.block_first_unmask_steps, self.global_denoising_step, self.num_tokens_backend, self.sparsity_params,self.block_length) = state
