...
def _resolve_model_path(path: str) -> str:
    ...
    raise FileNotFoundError(
        f"Model weights must be available as a local directory, got {path!r}. "
        "Download the model first, then pass MODEL=/path/to/TraDo-8B-Thinking."
    )
...
def load_model(model: nn.Module, path: str):
    ...
    weight_files = sorted(glob(os.path.join(model_path, "*.safetensors")))
    if not weight_files:
        raise FileNotFoundError(
            f"No safetensors weights found for model={path!r} resolved to {model_path!r}"
        )
