...
from generatev2 import block_prefill_teacher_student
...
def compute_loss_by_config(
    ...
    if training_mode == 'llada':
        ...
        logits_dense, logits_sparse, loss,acc, acc_dense, kl_loss, kl_loss_dense=block_prefill_teacher_student(
            teacher_denoiser,
            denoiser,
            input_ids=input_ids,
            mask_id=mask_id,
            block_length=block_size,
            denoising_steps=train_cfg.get("denoising_steps_per_block", 8),
            remasking_strategy=train_cfg.get("remasking_strategy", "low_confidence_dynamic"),
            confidence_threshold=train_cfg.get("dynamic_threshold", 0.9),
            tokenizer=tokenizer,
            prompt_lengths=question_length,
            train_sparsity_ratio=train_cfg.get("train_sparsity_ratio", 100),
            sparse_loss_weight=train_cfg.get("sparse_loss_weight", 0.5),
            dense_loss_weight=train_cfg.get("dense_loss_weight", 0.5),
            multi_block_probe=train_cfg.get("multi_block_probe", False),
            multi_block_probe_num=train_cfg.get("multi_block_probe_num", 0),
            multi_block_min_prefix_blocks=train_cfg.get("multi_block_min_prefix_blocks", 1),
            multi_block_random_probe=train_cfg.get("multi_block_random_probe", denoiser.training),
        )
        return {
            "loss" : loss,
            "acc": acc,
