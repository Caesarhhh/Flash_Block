...
struct BlockInfo {
    ...
    __device__ BlockInfo(const Params &params, const int bidb)
        ...
        , delta_seqlen_k_cache((!Varlen || params.cu_delta_seqlens_k == nullptr ? 0 : (params.is_seqlens_k_cumulative ? params.cu_delta_seqlens_k[bidb + 1] - sum_s_k : params.cu_delta_seqlens_k[bidb])) - leftpad_k)
        ...
        , delta_actual_seqlen_k(params.seqused_k ? params.seqused_k[bidb] - leftpad_k : delta_seqlen_k_cache)
        {
        }
    ...
    const int delta_seqlen_k_cache;
    const int actual_seqlen_k;
    const int delta_actual_seqlen_k;
};
