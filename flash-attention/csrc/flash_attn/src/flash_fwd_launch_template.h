...
DEFINE_FLASH_FORWARD_KERNEL(
    ...
    // ✅ 只在有 block_size 时调用新实现
    if (params.block_size > 0) {
        FLASH_NAMESPACE::compute_attn_with_history<
            Kernel_traits, Is_dropout, Is_causal, Is_local,
            Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Return_softmax
        >(params);
    } else {
...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_hist_only_kernel, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV) {

    #if defined(ARCH_SUPPORTS_FLASH)
        if (params.block_size > 0){
            FLASH_NAMESPACE::compute_attn_splitkv_with_history_hist_only<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params);
        }
        else{
            FLASH_NAMESPACE::compute_attn_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params);
        }
...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_combine_kernel, int kBlockM, int Log_max_splits, bool Is_even_K) {
    ...
    // printf("params.block_size %d",params.block_size);
    if (params.block_size > 0){
        // printf("flash_fwd_splitkv_combine_kernel params.block_size %d\n",params.block_size);
        // printf("combine_attn_seqk_parallel_with_history\n");
        FLASH_NAMESPACE::combine_attn_seqk_parallel_with_history<Kernel_traits, kBlockM, Log_max_splits, Is_even_K>(params);
    }else{
                    ...
                    BOOL_SWITCH(params.knew_ptr != nullptr, Append_KV, [&] {
                        ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                            SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                                ...
                                // If not IsEvenKConst, we also set IsEvenMNConst to false to reduce number of templates.
                                // If Is_local, set Is_causal to false
                                auto kernel = params.block_size > 0
                                    ? &flash_fwd_splitkv_hist_only_kernel<Kernel_traits, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && !Append_KV && IsEvenKConst && !Is_local && !Has_alibi && Kernel_traits::kHeadDim <= 128, IsEvenKConst && !Has_alibi, Is_softcap, Split, Append_KV>
                                    : &flash_fwd_splitkv_kernel<Kernel_traits, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && !Append_KV && IsEvenKConst && !Is_local && !Has_alibi && Kernel_traits::kHeadDim <= 128, IsEvenKConst && !Has_alibi, Is_softcap, Split, Append_KV>;
