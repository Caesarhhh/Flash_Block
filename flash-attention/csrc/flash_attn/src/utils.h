...
template<typename Tensor0, typename Tensor1, typename Tensor2, typename Tensor3,
         typename TiledMma, typename TiledCopy, typename ThrCopy>
__forceinline__ __device__ void gemm_rs_with_hist(
    Tensor0 &acc, Tensor0 &acc_hist,
    Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsB,
    TiledMma tiled_mma, TiledCopy smem_tiled_copy_B, ThrCopy smem_thr_copy_B,
    int k_start, int kBlockN, int hist_boundary)
{
    ...
    for (int i = 0; i < size<2>(tCrA); ++i) {
        ...
        // 临时片段存储 Δacc
        Tensor delta_acc = make_fragment_like(acc);
        clear(delta_acc);
        cute::gemm(tiled_mma, tCrA(_, _, i), tCrB(_, _, i), delta_acc);
        ...
        // 如果在历史区间，则累加到 acc_hist
        int k_tile_start = k_start + i * (kBlockN / size<2>(tCrA));
        int k_tile_end   = k_tile_start + (kBlockN / size<2>(tCrA));
        if (k_tile_start < hist_boundary) {
            #pragma unroll
            for (int j = 0; j < size(acc_hist); ++j)
                acc_hist(j) += delta_acc(j);
        }
    }
}
