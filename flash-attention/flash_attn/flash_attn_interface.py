...
def _flash_attn_varlen_forward(
    ...
    attn_output_past: Optional[torch.Tensor] = None,
    logsumexp: Optional[torch.Tensor] = None,
    need_update_mask: Optional[torch.Tensor] = None,
    dirty_mask: Optional[torch.Tensor] = None,
    block_size: int = 0
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    output = flash_attn_gpu.varlen_fwd(
        q,
        k,
        v,
        None,
    ...
    #     breakpoint()
    empty0 = q.new_empty((0,))
    empty1 = q.new_empty((0,))
    if block_size==0:
        out, softmax_lse, S_dmask, rng_state = output
        return out, softmax_lse, S_dmask, rng_state, empty0, empty1
    else:
        if attn_output_past is not None:
            out, softmax_lse, output_hist, softmax_lse_hist, S_dmask, rng_state, attn_output_past, logsumexp = output
            return torch.stack([out,output_hist]), torch.stack([softmax_lse, softmax_lse_hist]), S_dmask, rng_state, attn_output_past, logsumexp
        out, softmax_lse, output_hist, softmax_lse_hist,  S_dmask, rng_state = output
        return torch.stack([out,output_hist]), torch.stack([softmax_lse, softmax_lse_hist]),  S_dmask, rng_state, empty0, empty1
    return out, softmax_lse, S_dmask, rng_state
...
def _flash_attn_varlen_forward_fake(
    ...
    rng_state = torch.empty((2,), dtype=torch.int64, device=q.device)
    return out, softmax_lse, p, rng_state, q.new_empty((0,)), q.new_empty((0,))
...
class FlashAttnVarlenQKVPackedFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if head_size_og % 8 != 0:
            ...
            k = torch.nn.functional.pad(k, [0, 8 - head_size_og % 8])
            v = torch.nn.functional.pad(v, [0, 8 - head_size_og % 8])
        out_padded, softmax_lse, S_dmask, rng_state, _, _ = _wrapped_flash_attn_varlen_forward(
            q,
            k,
            v,
            cu_seqlens,
...
class FlashAttnVarlenFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if block_size>0:
            if attn_output_past is not None:
                out_all, softmax_lse_all, S_dmask, rng_state, attn_output_past, logsumexp = _wrapped_flash_attn_varlen_forward(
                    q,
                    k,
                    v,
                    cu_seqlens_q,
                    ...
                    head_mask=head_mask,
                    attn_output_past=attn_output_past,
                    logsumexp=logsumexp,
                    need_update_mask=need_update_mask,
                    dirty_mask=dirty_mask,
                    block_size=block_size
                )
            else:
                out_all, softmax_lse_all, S_dmask, rng_state, _, _ = _wrapped_flash_attn_varlen_forward(
                    q,
                    k,
                    v,
                    cu_seqlens_q,
            ...
            out_padded, out_padded_hist = out_all
            softmax_lse, softmax_lse_hist = softmax_lse_all
        else:
            out_padded, softmax_lse, S_dmask, rng_state, _, _ = _wrapped_flash_attn_varlen_forward(
                q,
                k,
        ...
        out = out_padded[..., :head_size_og]
        if block_size>0:
            out_hist = out_padded_hist[..., :head_size_og]
            if attn_output_past is not None:
                return (out, softmax_lse, out_hist, softmax_lse_hist, S_dmask, attn_output_past, logsumexp)
            return (out,out_hist) if not return_softmax else (out, softmax_lse,out_hist,softmax_lse_hist, S_dmask)
        return out if not return_softmax else (out, softmax_lse, S_dmask)
...
def flash_attn_varlen_func(
    ...
    block_table=None,
    head_mask=None,
    attn_output_past=None,
    logsumexp=None,
    need_update_mask=None,
    dirty_mask=None,
    block_size=0,
):
    """dropout_p should be set to 0.0 during evaluation
...
def flash_attn_with_kvcache(
    ...
    logsumexp: Optional[torch.Tensor] = None,
    dirty_mask: Optional[torch.Tensor] = None,
    need_update_kvcache_mask: Optional[torch.Tensor] = None,
    need_store_history_mask: Optional[torch.Tensor] = None,
    delta_cache_seqlens: Optional[Union[(int, torch.Tensor)]] = None,
):
    """
    ...
    output = flash_attn_gpu.fwd_kvcache(
        q,
        k_cache,
        v_cache,
        k,
        ...
        logsumexp,
        dirty_mask,
        need_update_kvcache_mask,
        need_store_history_mask,
        delta_cache_seqlens,
        block_size,
    )
    if block_size>0:
        if attn_output_past is not None:
            out, softmax_lse, out_hist, softmax_lse_hist, attn_output_past, logsumexp = output
            return (out, softmax_lse, out_hist, softmax_lse_hist,attn_output_past,logsumexp) if return_softmax_lse else (out, out_hist)
        else:
            out, softmax_lse, out_hist, softmax_lse_hist = output
            return (out, softmax_lse, out_hist, softmax_lse_hist) if return_softmax_lse else (out, out_hist)
    else:
