...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            flow_pred,att_time_all = self.model(
            ...
            kwargs = {}
            if isinstance(self.model, CausalWanModel):
                kwargs['sink_recache_after_switch'] = sink_recache_after_switch
            ...
            if clean_x is not None:
                ...
                if isinstance(out, tuple):
                    flow_pred, att_time_all = out[0], out[1]
                else:
                    flow_pred, att_time_all = out, 0
                flow_pred = flow_pred.permute(0, 2, 1, 3, 4)
            else:
                if classify_mode:
                    out = self.model(
                    ...
                    if isinstance(out, tuple) and len(out) == 3:
                        flow_pred, logits, att_time_all = out
                    elif isinstance(out, tuple) and len(out) == 2:
                        flow_pred, logits = out
                        att_time_all = 0
                    else:
                        flow_pred, logits, att_time_all = out, None, 0
                    flow_pred = flow_pred.permute(0, 2, 1, 3, 4)
                else:
                    out = self.model(
                        noisy_image_or_video.permute(0, 2, 1, 3, 4),
                    ...
                    if isinstance(out, tuple):
                        ...
                        flow_pred, att_time_all = out, 0
                    flow_pred = flow_pred.permute(0, 2, 1, 3, 4)
        # if not sparsity_params["frame_idx"] in self.flow_preds:
        #     self.flow_preds[sparsity_params["frame_idx"]] = {}
        #     self.pred_x0s[sparsity_params["frame_idx"]] = {}
        #     self.noisy_image_or_videos[sparsity_params["frame_idx"]] = {}
        # self.flow_preds[sparsity_params["frame_idx"]][sparsity_params["denoising_step"]]=flow_pred
        pred_x0 = self._convert_flow_pred_to_x0(
            flow_pred=flow_pred.flatten(0, 1),
            xt=noisy_image_or_video.flatten(0, 1),
            timestep=timestep.flatten(0, 1)
        ).unflatten(0, flow_pred.shape[:2])
        # self.pred_x0s[sparsity_params["frame_idx"]][sparsity_params["denoising_step"]]=pred_x0
        # self.noisy_image_or_videos[sparsity_params["frame_idx"]][sparsity_params["denoising_step"]]=noisy_image_or_video

        if logits is not None:
            return flow_pred, pred_x0, logits

        return flow_pred, pred_x0, att_time_all
